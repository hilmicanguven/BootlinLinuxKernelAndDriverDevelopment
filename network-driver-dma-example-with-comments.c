 ...
 *
 * Explanations added through // comments by Michael Opdenacker <michael@bootlin.com>
 * Original file:  drivers/net/ethernet/rdc/r6040.c
 * See all our technical docs on https://bootlin.com/docs/
 *
 * We advise you to read this file starting from the probe and remove
 * functions at the bottom, and progressively going up to lower level functions.o
 *
...
static int r6040_rx(struct net_device *dev, int limit)
{
	...
	while (count < limit && !(descptr->status & DSC_OWNER_MAC)) {
		...
		// Give ownership of the new descriptor buffer to the device (for DMA)
...
/* Init RDC MAC */
// Called in the open function - Gets the device ready to transmit and receive
...
// Open function - Called when ethx is opened
...
`		// Failed. Free the rx ring before exiting
	...
	// Initialize MAC, alloc and initialize tx and rx buffers
	ret = r6040_up(dev);
	if (ret)
...
// Declared by attaching to the net_device_ops structure
				    ...
				    struct net_device *dev)
{
	...
	/* Critical Section */
	// Prevent concurrent access to device registers, descriptors
	// driver statistic and driver private data
...
static const struct ethtool_ops netdev_ethtool_ops = {
	 ...
	 // ethx from userspace, setting half/full duplex for example
...
static const struct net_device_ops r6040_netdev_ops = {
	.ndo_open		= r6040_open,		// Called when ethx is opened
	.ndo_stop		= r6040_close,		// Called when ethx is closed
	.ndo_start_xmit		= r6040_start_xmit,	// Called when a packet is emitted
							// by the network stack
	.ndo_get_stats		= r6040_get_stats,	// Device rx and tx count stats
...
static int r6040_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	// Device probe function - Called when the bus finds a new device
	...
	int bar = 0;			  // Base address register on the PCI bus (BAR0)
	u16 *adrp;

	pr_info("%s\n", version);

	// Enable the PCI device. This makes device configuration accessible.
	// An IRQ line is also assigned then (if not done yet by the BIOS)
	...
	// The below two calls check that the PCI controller
        // on the machine can properly support the DMA addressing
	// limitations that the your device has.
	// See Documentation/DMA-mapping.txt for explanations
	...
	// Allow the device to become a master on the bus (to do DMA)
	...
	// This way, you can get a handle on the PCI device in routines that
	// only have access to the net device.
	// That's needed to access hardware registers!
	...
	// Convenient for quick access to device PCI registers
	...
	// Attach the device net and ethtool specifics ops
...
static void r6040_remove_one(struct pci_dev *pdev)
{
	// Device remove function - Run when a remove event is received
        // It's called "one" because it removes one device

  	// Retrieve the net device from the physical device
        // This will allow to unregister the network device
        // from the system

	struct net_device *dev = pci_get_drvdata(pdev);
	struct r6040_private *lp = netdev_priv(dev);

	// Basically undoing what was registered and allocated
        // in the probe function.
        // See the probe function for explanations
...
static const struct pci_device_id r6040_pci_tbl[] = { // Table of supported devices
...
static struct pci_driver r6040_driver = {
	...
	.probe		= r6040_init_one, // Loaded when the PCI bus detects a matching device
	.remove		= r6040_remove_one,
};

// Shortcut to declare PCI module init and exit functions
// which just register the above pci_driver structure
